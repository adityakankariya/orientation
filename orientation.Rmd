...
The data-set can be found as `data_cleaned.csv`.
...
```{r}
Gender <-table(sub_UKBB$f.22001.0.0)
Gender
barplot(Gender,main="Gender Diversity")
...
**Answer**
In general conditions, gender is considered as a binary trait. Because here only there are two values male or female. 
...
**Answer**
For statistical analysis, we cant use the strings as input. So we need to code the responses into valid numbers. So we have recorded the responses into the numerical values. Here we replaced "No" by using the value "0". Similarly the values like "Yes, but not now, but have in the past", "Yes, now some of the time", "Yes, now a lot of the time"= 1, "Yes, now most or all of the time" are replaced by value "1". And the values like "Prefer not to answer", "Do not know" are replaced by the value "NA".
...
Here there are two different groups or cases are founded and they are true and false. The dataset contains five controls. 
...
This dataset is related to the disease dataset. Here the first age may be the age when the patient or respondent first noticed the problems or symptoms of the disease. And the second age may be the current age of the patient. So in some cases, both are same, which means they are considered as a new patient. They only noticed the problem or symptoms of the disease now. Other having the problems over the specific period of time. 

**Q7:** For those with more than one age records, which age do you think should be used in the genetic association analysis?

**Answer**

We need to consider the age when they noticed the symptoms of the particular disease or problem. So we need to consider the age given first or smaller age between the two ages. in the given case the age field f.21003.0.0 needs to be used. 


**Q8:** Please compute a summary of age information for controls (you can use `summary()` function in R):
```{r}
#YOUR CODE
summary(sub_UKBB$f.21003.0.0)
```
\newpage
...
```
**Answer**
From the finds of the research, the p-value is found as "2.2e-16" this value is less than 0.05 at 95 % of the significance level. So we reject the null hypothesis. Which means there is no evidence to prove there is no relation between the two variables. And the correlation coefficient between the two variables is calculated as -0.007647. So we can say that there is a negative relation between the mph as well as weight. 
...
```{r}
auto_data
newvariable <- ifelse(auto_data$mpg < 23, "0", "1")
newvariable=as.integer(newvariable)
auto_data1 <-data.frame(auto_data,newvariable)
auto_data1

lr.auto_data1 = glm(formula = newvariable ~ weight, data = auto_data1, family = binomial)
print(summary(lr.auto_data1))
```

**Answer:**
Above given results shows the results of logistic regression conducted on the modified auto_car dataset. 
...
#linear regression
linearRegression=lm(formula = newvariable ~ weight, data = auto_data1)
linearRegression
